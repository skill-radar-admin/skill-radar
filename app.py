...
def generate_badge_image(combat_power, qual_count, synergy_count, badge_name, user_email):
    ...
    # 画像文字化け対策: 絵文字と日本語を消してスタイリッシュな英語（大文字）に変換
    ...
    # 左下に日付とシリアルを配置
    draw.text((40, 350), f"Issued: {date_str}", fill=(148, 163, 184), font=font_small)
    draw.text((40, 380), f"Serial: SR-{serial_no}", fill=(148, 163, 184), font=font_small)
    
    # 右下の公式スタンプ風デザイン (ptに被らないよう四角枠に変更し、位置を右下に固定)
    stamp_box = [(620, 350), (760, 400)]
    draw.rectangle(stamp_box, outline=(56, 189, 248), width=2)
    bbox = draw.textbbox((0, 0), "VERIFIED", font=font_small)
    tw = bbox[2] - bbox[0]
    th = bbox[3] - bbox[1]
    box_center_x = (stamp_box[0][0] + stamp_box[1][0]) / 2
    box_center_y = (stamp_box[0][1] + stamp_box[1][1]) / 2
    draw.text((box_center_x - tw/2, box_center_y - th/2 - 2), "VERIFIED", fill=(56, 189, 248), font=font_small)
